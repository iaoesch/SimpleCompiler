...
public:
  ...
  //  void Set(const std::vector<std::shared_ptr<VariableClass> > &Parameters, LocationType const &Loc);
    ...
    void BeginConstructorMethodCallForObject(std::string ClassName, std::string MethodName, const LocationType &Loc);
    ...
    std::shared_ptr<InstanceConstructionClass> FinishConstructorMethodCall(const LocationType &Loc);
